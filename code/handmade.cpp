...
extern "C" GAME_UPDATE_AND_RENDER(GameUpdateAndRender) {
  ...
  for (int ControllerIndex = 0;
    ...
    if (GameState->tJump > 0) {
      GameState->PlayerY += (int)(5.0 * sinf(0.5f * Pi32 * GameState->tJump));
    }
  ...
  for (int BufferIndex = 0; BufferIndex < ArrayCount(Input->MouseButtons);
       BufferIndex++) {
    if (Input->MouseButtons[BufferIndex].EndedDown) {
      RenderPlayer(Buffer, 10 + 20 * BufferIndex, 10);
    }
  }
}
